...
def initialize_task(experiment: LightweightExperimentConfig, model: BaseLLM) -> BaseTask:
    ...
    num_responses = math.ceil(experiment.num_responses / num_samples_per_prompt)
    print(f"{experiment.name} experiment targeting: {experiment.num_responses} desired / {num_samples_per_prompt} per prompt = {num_responses} responses")
    ...
    for i, model in enumerate(models, 1):
        ...
        if args.core:
            results = run_core_generation(
                task=Task.CREATIVE_STORY, num_responses=10, model_name=model, method=method, temperature=0.7, top_p=1.0,
            ...
            run_generation_test(task=Task.CREATIVE_STORY, num_responses=10, model_name=model, method=method, temperature=0.7,
